...
import (
	...
	"delivery/auth_service/helper"
	t "delivery/auth_service/token"
)
...
func (ur *UserRepo) RegisterCourier(req *pb.UserCreateReq) (*pb.Void, error) {
	query := `insert into users(id,
	                            username, 
								email, 
								password, 
...
func (r *UserRepo) UpdateProfile(req *pb.UserUpdate) (*pb.Void, error) {
	...
	if req.Body.PrivacyLevel != "" && req.Body.PrivacyLevel != "string" {
		cons = append(cons, fmt.Sprintf("privacy_level=$%d", len(args)+1))
		args = append(args, req.Body.PrivacyLevel)
	}
	if req.Body.Theme != "" && req.Body.Theme != "string" {
		cons = append(cons, fmt.Sprintf("theme=$%d", len(args)+1))
		args = append(args, req.Body.Theme)
	}
	if req.Body.NotificationEnable != "" && req.Body.NotificationEnable != "string" {
		cons = append(cons, fmt.Sprintf("notification_enable=$%d", len(args)+1))
		args = append(args, req.Body.NotificationEnable)
	}
	...
	if req.Body.Working != "" && req.Body.Working != "string" {
		cons = append(cons, fmt.Sprintf("working=$%d", len(args)+1))
		args = append(args, req.Body.Working)
	}
...
func (r *UserRepo) CreateManager(req *pb.UserCreateReq) (*pb.Void, error) {
	query := `insert into users(id,
	username, 
	email, 
	password, 
...
func (r *UserRepo) GetAllUsers(req *pb.UserFilter) (*pb.UserList, error) {
	...
	if req.IsWorking != "" && req.IsWorking != "string" {
		cons = append(cons, fmt.Sprintf("working=$%d", len(args)+1))
		if req.IsWorking == "true" {
			args = append(args, "yes")
		} else if req.IsWorking == "false" {
			args = append(args, "no")
		}
	}
